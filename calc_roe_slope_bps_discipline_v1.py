...
FACTOR_ID = 'roe_slope_bps_discipline_v1'
...
fund['roe_slope2'] = (fund['roe'] - fund['roe_lag2']) / 2.0
fund['roe_accel'] = (fund['roe'] - fund['roe_lag1']) - (fund['roe_lag1'] - fund['roe_lag2'])
fund['bps_yoy_mean2'] = g['bps_yoy'].transform(lambda s: s.rolling(2, min_periods=2).mean())

level_gate = np.tanh(fund['roe_mean4'].fillna(0) / 7.5)
stability = 1.0 / (1.0 + 0.8 * fund['roe_std4'].abs())
capital_discipline = 1.0 / (1.0 + 7.5 * fund['bps_yoy_mean2'].clip(lower=0).fillna(0).abs() + 4.0 * fund['bps_qoq'].clip(lower=0).fillna(0).abs())
inflect = np.tanh(fund['roe_slope2'].fillna(0) / 2.5) + 0.7 * np.tanh(fund['roe_accel'].fillna(0) / 2.0)
quality_of_growth = np.tanh((fund['roe_yoy'].fillna(0) - 45.0 * fund['bps_yoy'].fillna(0)) / 5.5)
fund['raw_factor'] = (0.40 * inflect + 0.40 * quality_of_growth + 0.20 * level_gate) * stability * capital_discipline
