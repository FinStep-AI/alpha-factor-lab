...
FACTOR_ID = 'roe_improve_efficiency_v1'
...
fund['bps_accel'] = g['bps'].transform(lambda s: s.pct_change().diff())

# 核心直觉：ROE同比改善如果不是靠激进净资产扩张“堆”出来，质量更高
profit_improve = fund['roe_yoy']
stability = fund['roe_ma4'] / (1.0 + fund['roe_std4'].abs())
capital_discipline = 1.0 / (1.0 + 4.0 * fund['bps_yoy'].abs() + 2.0 * fund['bps_accel'].abs().fillna(0))

fund['raw_factor'] = profit_improve * np.sign(stability) * np.log1p(stability.abs().clip(lower=0)) * capital_discipline
