...
FACTOR_ID = 'roe_reaccel_lowexp_smooth_v1'
...
fund['roe_prev_qoq'] = fund['roe_lag1'] - fund['roe_lag2']
fund['roe_reaccel'] = fund['roe_qoq'] - fund['roe_prev_qoq']
...
profit_level = np.tanh(fund['roe_mean4'].fillna(0) / 9.0)
profit_trend = np.tanh(fund['roe_yoy_mean2'].fillna(0) / 6.0)
reaccel = np.tanh(fund['roe_reaccel'].fillna(0) / 3.0)
stability_gate = 1.0 / (1.0 + 0.8 * fund['roe_std4'].abs().fillna(0))
lowexp_reward = -np.tanh(3.0 * fund['bps_yoy_mean2'].clip(lower=0).fillna(0) + 1.5 * fund['bps_qoq'].clip(lower=0).fillna(0))

fund['raw_factor'] = (0.38 * reaccel + 0.34 * profit_trend + 0.28 * profit_level + 0.32 * lowexp_reward) * stability_gate
