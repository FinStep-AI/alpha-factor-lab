...
FACTOR_ID = 'asset_turnover_reaccel_quality_v1'
...
fund['equity_proxy'] = fund['bps'].clip(lower=0.1)
fund['asset_turnover_proxy'] = fund['roe'] / fund['equity_proxy']
fund['at_lag1'] = g['asset_turnover_proxy'].shift(1)
fund['at_lag4'] = g['asset_turnover_proxy'].shift(4)
fund['at_lag8'] = g['asset_turnover_proxy'].shift(8)
...
fund['at_yoy'] = fund['asset_turnover_proxy'] - fund['at_lag4']
fund['at_qoq'] = fund['asset_turnover_proxy'] - fund['at_lag1']
fund['at_reaccel'] = (fund['asset_turnover_proxy'] - fund['at_lag4']) - (fund['at_lag4'] - fund['at_lag8'])
fund['roe_yoy'] = fund['roe'] - fund['roe_lag4']
fund['roe_qoq'] = fund['roe'] - fund['roe_lag1']

turnover_level = np.tanh(12.0 * fund['asset_turnover_proxy'].fillna(0))
turnover_improve = np.tanh(18.0 * fund['at_yoy'].fillna(0) + 8.0 * fund['at_qoq'].fillna(0))
turnover_reaccel = np.tanh(20.0 * fund['at_reaccel'].fillna(0))
profit_confirm = np.tanh((0.7 * fund['roe_yoy'].fillna(0) + 0.3 * fund['roe_qoq'].fillna(0)) / 5.0)
quality_gate = np.tanh(fund['roe_mean4'].fillna(0) / 8.0)
stability_gate = 1.0 / (1.0 + 0.12 * fund['roe_std4'].abs().fillna(0) + 0.18 * fund['bps_std4'].abs().fillna(0))
expansion_penalty = 1.0 / (1.0 + 8.0 * g['bps'].pct_change().clip(lower=0).fillna(0).abs())

fund['raw_factor'] = (
    0.28 * turnover_level +
    0.30 * turnover_improve +
    0.22 * turnover_reaccel +
    0.20 * profit_confirm
) * (0.55 + 0.45 * quality_gate) * stability_gate * expansion_penalty
