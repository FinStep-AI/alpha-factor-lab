...
FACTOR_ID = 'roe_persistence_gap_v3'
...
fund['roe_persistence'] = fund['roe_qoq'] + 0.8 * fund['roe_prev_qoq']
fund['roe_consistency'] = - (fund['roe_qoq'] - fund['roe_prev_qoq']).abs()
...
profit_improve = np.tanh(fund['roe_yoy'].fillna(0) / 5.5)
persistence = np.tanh(fund['roe_persistence'].fillna(0) / 3.0)
consistency = np.tanh(fund['roe_consistency'].fillna(0) / 2.0)
stability_gate = 1.0 / (1.0 + 0.9 * fund['roe_std4'].abs().fillna(0))
capital_penalty = np.tanh(4.0 * fund['bps_yoy_mean2'].clip(lower=0).fillna(0) + 2.0 * fund['bps_qoq'].clip(lower=0).fillna(0))
