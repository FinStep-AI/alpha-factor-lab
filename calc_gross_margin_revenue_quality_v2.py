...
FACTOR_ID = 'gross_margin_revenue_quality_v2'
...
fund['gm_yoy_std4'] = g['gross_income_ratio_yoy'].transform(lambda s: s.rolling(4, min_periods=3).std())
...
for c in [
    'gross_income_ratio','gm_yoy_ma2','rev_yoy_ma2','gm_yoy_std4','rev_yoy_std4',
    'inv_turn_delta','ar_ratio_delta','debt_delta','liq_buffer'
]:
    s = fund[c]
    q01, q99 = s.quantile([0.01, 0.99])
    fund[c] = s.clip(q01, q99)

margin_trend = np.tanh(fund['gm_yoy_ma2'] / 15.0)
revenue_trend = np.tanh(fund['rev_yoy_ma2'] / 25.0)
stability = 1.0 / (1.0 + 0.10 * fund['gm_yoy_std4'].abs().fillna(0) + 0.06 * fund['rev_yoy_std4'].abs().fillna(0))
working_capital_quality = np.tanh(fund['inv_turn_delta'].fillna(0) / 1.5) - 0.8 * np.tanh(fund['ar_ratio_delta'].fillna(0) / 6.0)
balance_sheet_gate = np.tanh((fund['liq_buffer'].fillna(0) - 1.0) / 0.8) - 0.5 * np.tanh(fund['debt_delta'].fillna(0) / 8.0)
level_anchor = np.tanh(fund['gross_income_ratio'].fillna(0) / 20.0)

fund['raw_factor'] = (
    0.35 * margin_trend +
    0.25 * revenue_trend +
    0.25 * working_capital_quality +
    0.15 * balance_sheet_gate
) * stability * (0.75 + 0.25 * level_anchor)
