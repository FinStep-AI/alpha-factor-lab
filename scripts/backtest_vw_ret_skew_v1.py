...
# -*- coding: utf-8 -*-
"""
因子: vw_ret_skew_v1 — 成交量加权收益率偏度
================================================
构造:
  1. 计算成交量加权的收益率偏度(20日窗口):
     weighted_ret = daily_ret * (volume / mean(volume))
     因子 = skewness(weighted_ret, 20)
     
  2. 市值中性化(OLS) + 5%缩尾 + z-score

逻辑:
- 传统偏度只看收益率大小, 本因子用成交量加权: 大成交日的偏度影响更大
- 正偏度: 大成交量日集中在上涨 → 知情交易者在关键日推动上涨 → 动量信号
- 负偏度: 大成交量日集中在下跌 → 恐慌/大资金出逃 → 下跌惯性
- 本质上是把"大成交日的方向信号"作为alpha来源

与现有因子区别:
- vs tail_risk_cvar(尾部风险): CVaR只看最差的几天, 本因子看整体分布形状
- vs amp_level(振幅): 振幅看绝对值波动, 本因子看方向分布
- vs vol_wt_skew: 已有因子但查看下是否用raw returns而非volume加权
"""
...
# ────────────────── 因子构造 ──────────────────
print(f"[2] 构造 Volume-Weighted Return Skewness 因子 (window={WINDOW})...")

# 成交量标准化
df["vol_ratio"] = df["volume"] / (df.groupby("stock_code")["volume"].transform(lambda x: x.rolling(WINDOW, min_periods=15).mean()) + 1)

# 成交量加权收益率
df["weighted_ret"] = df["daily_ret"] * df["vol_ratio"]

# 滚动偏度
def calc_skew(series):
    """计算滚动偏度，最小15个观测值"""
    return series.rolling(WINDOW, min_periods=15).skew()

print("    计算滚动偏度...")
df["factor_raw"] = df.groupby("stock_code")["weighted_ret"].transform(calc_skew)
...
mktcap_piv = df.pivot_table(index="date", columns="stock_code", values="avg_amount_20d", dropna=False)
log_mktcap = np.log(mktcap_piv.clip(lower=1))
...
if valid_results:
    ...
    report = {
        "factor_id": FACTOR_ID,
        "factor_name": "成交量加权收益率偏度 v1",
        "factor_name_en": "Volume-Weighted Return Skewness v1",
        "direction": bd,
        "forward_days": bf,
        "rebalance_freq": bf,
