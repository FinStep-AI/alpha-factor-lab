...
# -*- coding: utf-8 -*-
"""
因子: down_volume_ratio_v1 — 成交量恐慌比例
=============================================
构造:
  1. 过去20日:
     down_vol = 所有下跌日的成交量之和
     total_vol = 过去20日总成交量
     ratio = down_vol / total_vol

  2. 因子 = ratio （正向使用）
     高ratio = 下跌日成交量占比高 = 恐慌抛售集中 = 卖压释放后有望反弹

  3. 市值中性化(OLS) + 5%缩尾 + z-score

逻辑:
- 高 down_volume_ratio = 下跌日成交量占比大 = 市场恐慌/止损集中释放 → 卖压耗尽后反弹
- 低 down_volume_ratio = 下跌日成交量清淡 = 缺乏恐慌 = 可能继续阴跌
- 本质是成交量层面的短期反转信号

与现有因子区别:
- vs neg_day_freq(极端负收益频率): neg_day_freq看极端事件频率, 本因子看成交量分布
- vs tail_risk_cvar(尾部风险): CVaR看收益率CVaR, 本因子看成交量恐慌集中度
- vs turnover_decel(换手减速): turnover_decel看换手趋势, 本因子看方向性成交量分布
"""
...
COST = 0.003
WINSORIZE_PCT = 0.05
FACTOR_ID = "down_volume_ratio_v1"
...
# ────────────────── 因子构造(向量化) ──────────────────
print(f"[2] 构造 Down Volume Ratio 因子 (window={WINDOW})...")

df["down_vol_sum"] = df.groupby("stock_code")["down_volume"].transform(
    lambda x: x.rolling(WINDOW, min_periods=15).sum()
)
df["total_vol_sum"] = df.groupby("stock_code")["volume"].transform(
    lambda x: x.rolling(WINDOW, min_periods=15).sum()
)
df["factor_raw"] = df["down_vol_sum"] / (df["total_vol_sum"] + 1)
...
m1, g1, i1, r1, cd1, cs1, v1 = run_backtest(factor_df, 1, "正向(做多恐慌抛售后)")
m2, g2, i2, r2, cd2, cs2, v2 = run_backtest(factor_df, -1, "反向(做空恐慌抛售后)")
...
if valid_results:
    ...
    report = {
        "factor_id": FACTOR_ID,
        "factor_name": "成交量恐慌比例 v1",
        "factor_name_en": "Down Volume Ratio v1",
        "direction": bd,
        "forward_days": bf,
        "rebalance_freq": bf,
