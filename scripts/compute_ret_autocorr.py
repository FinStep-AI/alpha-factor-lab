#!/usr/bin/env python3
"""
因子：收益率自相关 (Return Autocorrelation)
ID: ret_autocorr_v1

逻辑：
- 20日滚动corr(ret_t, ret_{t-1}) = 一阶自相关
- 正自相关 = 趋势股（动量延续）
- 负自相关 = 反转股（均值回复）
- A股小盘股信息传播慢，可能正自相关（趋势延续）更有效
- 也可能负自相关（反转）更有效
- 先做正向回测（高自相关=趋势=做多），不达标再反向
...
def compute_factor(kline_path, output_path):
    ...
    df['daily_ret_lag'] = df.groupby('stock_code')['daily_ret'].shift(1)
    
    window = 20
    print("Computing return autocorrelation (20d rolling)...")
    
    g = df.groupby('stock_code')
    
    # 20日滚动自相关
    # 用pandas rolling corr
    df['ret_autocorr'] = df.groupby('stock_code').apply(
        lambda x: x['daily_ret'].rolling(window, min_periods=15).corr(x['daily_ret_lag']),
        include_groups=False
    ).reset_index(level=0, drop=True)
    
    df['factor_raw'] = df['ret_autocorr']
...
if __name__ == '__main__':
    ...
    compute_factor(
        ...
        os.path.join(base, 'data', 'factor_ret_autocorr_v1.csv')
    )
