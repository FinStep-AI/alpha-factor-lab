...
# -*- coding: utf-8 -*-
"""
BPS同比增长率因子 (bps_growth_v1)

逻辑：
  计算每只股票的每股净资产(BPS)同比增长率。
  BPS增长反映公司内生增长能力（留存收益积累）。
  
  BPS_growth = (BPS_t - BPS_{t-4Q}) / |BPS_{t-4Q}|
  
  BPS稳定增长 → 公司盈利质量高、内生增长强
  
  反向因子测试：既测正向（高增长→高收益），也准备反向
...
for stock, grp in fund.groupby('stock_code'):
    ...
    for i, row in grp.iterrows():
        ...
        if len(prev) == 0:
            continue
        prev_bps = prev.iloc[0]['bps']
        curr_bps = row['bps']
        if pd.isna(prev_bps) or pd.isna(curr_bps) or abs(prev_bps) < 0.01:
            continue
        growth = (curr_bps - prev_bps) / abs(prev_bps)
        results.append({
            'stock_code': stock,
...
print(f"  计算完成: {len(delta_df)} 条BPS增长率记录")
print(f"  均值: {delta_df['bps_growth'].mean():.4f}, 中位数: {delta_df['bps_growth'].median():.4f}")
...
for stock in stocks:
    ...
    if len(stock_deltas) == 0:
        continue
    right = stock_deltas[['avail_date', 'bps_growth']].rename(columns={'avail_date': 'date'}).sort_values('date')
...
factor_df = factor_df.dropna(subset=['bps_growth'])
...
def neutralize_cross_section(group):
    ...
    # Winsorize
    res_valid = pd.Series(residuals[mask])
    median = res_valid.median()
    mad = (res_valid - median).abs().median()
    if mad > 0:
...
output.to_csv('data/factor_bps_growth_v1.csv', index=False)
print(f"  输出: data/factor_bps_growth_v1.csv ({len(output)} 行)")
