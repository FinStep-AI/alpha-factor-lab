...
FACTOR_ID = 'roe_improve_quality_v1'
...
# extreme clip to stabilize small-cap quarterly noise
...
# paper-style intuition: persistent profitable improvement > noisy improvement
fund['raw_factor'] = fund['roe_yoy'] / (fund['roe_std4'].abs() + 1e-6)
# combine with profitability level to avoid low-quality turnarounds dominating
fund['raw_factor'] = fund['raw_factor'] * np.sign(fund['roe_mean4']) * np.log1p(fund['roe_mean4'].abs().clip(lower=0))
...
for stock, grp in factor_q.groupby('stock_code'):
    ...
    sf = sf.reindex(trade_dates, method='ffill', limit=60)
    sf['stock_code'] = stock
