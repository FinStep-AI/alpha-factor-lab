...
FACTOR_ID = 'roe_reaccel_bps_guard_v1'
...
fund['roe_yoy_prev'] = g['roe_yoy'].shift(1)
fund['roe_reaccel'] = fund['roe_yoy'] - fund['roe_yoy_prev']
...
fund['bps_yoy_abs_ma4'] = g['bps_yoy'].transform(lambda s: s.abs().rolling(4, min_periods=2).mean())

level_gate = np.sign(fund['roe_mean4']) * np.log1p(fund['roe_mean4'].abs().clip(lower=0))
stability = 1.0 / (1.0 + 0.6 * fund['roe_std4'].abs() + 0.8 * fund['roe_yoy_std4'].abs())
bps_guard = 1.0 / (1.0 + 3.0 * fund['bps_yoy_abs_ma4'].clip(lower=0).fillna(fund['bps_yoy_abs_ma4'].median()))
turnaround = np.tanh(fund['roe_yoy'].fillna(0) / 4.0)

fund['raw_factor'] = fund['roe_reaccel'] * level_gate * stability * bps_guard * (1.0 + 0.3 * turnaround)
