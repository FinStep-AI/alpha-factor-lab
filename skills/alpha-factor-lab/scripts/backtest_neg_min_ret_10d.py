#!/usr/bin/env python3
"""
因子: neg_min_ret_10d — 10日极端最低收益因子
============================================

方向: 反转/风险 (尚未覆盖：与CVaR不同，捕捉单日最极端下跌)

构造:
  1. 过去10日中最低单日收益率 min(ret, 10d)
  2. 取负值: -min_ret，使得极端负收益日得高因子值
  3. 20日成交额中性化 (OLS)
  4. 5%缩尾

逻辑:
  与 CVaR 的区别:
  - CVaR: 过去10日最差2日的平均收益 (平滑)
  - neg_min_ret: 过去10日最差1日的收益 (极端事件)

  假设:
  - extremely negative day in last 10d → panic/distress signal
  - 但在A股中证1000小盘股上，极端抛售后通常有短期反弹
  - (类似 neg_day_freq_v1 的逻辑，但用幅度而非频率)

理论:
  - Bali, Cakici & Whitelaw (2011) "Maxing Out": Stocks as Lotteries
  - 极端负收益 = 尾部事件 = 短期反转 (非延续)
  - 与 CVaR 高相关但捕捉不同的尾部特征
...
COST = 0.002
WINSORIZE_PCT = 0.05
DATA_CUTOFF = "2026-04-22"  # 最新数据
FACTOR_ID = "neg_min_ret_10d"

BASE_DIR = Path(__file__).resolve().parent  # 脚本就在 alpha-factor-lab/ 根目录
...
# ────────────────── 因子构造 ──────────────────
print(f"[2] 构造极端最低收益因子 (window={WINDOW}d)...")
...
for i in range(WINDOW, n_dates):
    window = ret_vals[i-WINDOW:i, :]  # (WINDOW, n_stocks)
    # 窗口内的最小收益率 (沿时间轴取最小值)
    min_ret = np.nanmin(window, axis=0)  # (n_stocks,)
    n_valid = np.sum(~np.isnan(window), axis=0)
    min_ret[n_valid < WINDOW * 0.7] = np.nan
    factor_raw_vals[i, :] = min_ret
...
if neg_sh > pos_sh:
    ...
    direction = -1
    direction_desc = "反向（不高极端负收益=高预期收益，反转逻辑）"
else:
    direction = 1
    direction_desc = "正向（高因子值=高预期收益，反转/反弹逻辑）"
print(f"   → 使用方向: {direction_desc}")

# ────────────────── 相关性 ──────────────────
print(f"[7] 与现有因子相关性...")

# Construct reference factors
...
ret_vals_full = ret_piv.values
for i in range(10, n_dates):
    w = ret_vals_full[i-10:i, :]
    s = np.sort(w, axis=0)
...
ic_m_final = metrics.get("ic_mean", 0) or 0
ic_t_final = metrics.get("ic_t_stat", 0) or 0

report = {
    "factor_id": FACTOR_ID,
    "factor_name": "10日极端最低收益 v1",
    "factor_name_en": "10-Day Minimum Return v1",
    "category": "反转/尾部风险",
    "description": f"过去{WINDOW}日中最低单日收益率(取负高值=极端下跌)。与CVaR(平均最差2日)互补，捕捉单日极端抛售信号。成交额OLS中性化。",
    "hypothesis": "过去10日经历过极端单日下跌(如-7%以上)的股票，在A股中证1000小盘股上短期内发生反弹(均值回复)。区别于CVaR的平滑尾部，本因子捕捉最极端事件。",
    "formula": f"neutralize(-min(ret, {WINDOW}d), log_amount_20d)",
    "direction": direction,
...
print(f"\n  ➤ 因子{'有效 ✓ 达标入库' if is_valid else '无效 ✗ 未达标'}")
print(f"  评估标准: |IC|>0.015, |t|>2, |Sharpe|>0.5")
if is_valid:
    print(f"  → 达标！更新 factors.json + git commit")
else:
    print(f"  → 记录失败原因")
