...
# -*- coding: utf-8 -*-
"""
因子: auction_eff_v1 — 开盘定价效率因子
==========================================

方向: 信息效率 / 微观结构

构造:
  1. 计算每日 |open - close| / close，衡量开盘价相对收盘价的偏差
  2. 20日滚动均值(MA20)
  3. 对数变换(因原始值高度右偏)
  4. 成交额OLS中性化
  5. 5%缩尾
...
逻辑:
  集合竞价定价偏差反映了开盘时的信息噪声程度:
  - 高偏差 = 集合竞价定价不准 = 信息不对称高 / 噪声交易多
    → 可能存在信息不对称风险溢价(做多)
    → 也可能后续价格波动大导致被高估(做空)
  - 低偏差 = 集合竞价定价准确 = 信息透明 / 机构定价主导
    → 可能信息充分无alpha(做空)
    → 也可能稳定性溢价(做多)
...
理论:
  - Biais, Hillion & Spatt (1999) "Price Discovery and Learning during the Preopening Period"
  - Cao, Ghysels & Hatheway (2000) "Price Discovery without Trading: Evidence from NASDAQ"
  - A股集合竞价机制: 9:15-9:25开盘集合竞价，包含大量知情/噪声交易信号
"""
...
# ────────────────── 因子构造 ──────────────────
print(f"[2] 构造开盘定价效率因子 (window={WINDOW})...")

# |open - close| / close: 日内开盘偏差率
oc_dev = (open_piv - close_piv).abs() / close_piv.clip(lower=0.01)

# 20日滚动均值
oc_dev_ma = oc_dev.rolling(WINDOW, min_periods=10).mean()

# 对数变换 (减少右偏)
factor_raw = np.log(oc_dev_ma.clip(lower=1e-8))
...
print(f"   正向 (高偏差=高收益): IC={pos_ic:.4f}, t={pos_t:.2f}, Sharpe={pos_sh:.4f}")
print(f"   反向 (低偏差=高收益): IC={neg_ic:.4f}, t={neg_t:.2f}, Sharpe={neg_sh:.4f}")

# 选择更好的方向 (综合IC和Sharpe)
pos_score = abs(pos_ic) * pos_sh if pos_sh > 0 else 0
neg_score = abs(neg_ic) * neg_sh if neg_sh > 0 else 0

if neg_score > pos_score:
    print(f"   → 使用反向 (做多高效率/低偏差)")
    direction = -1
    fa_final = -fa
    direction_desc = "反向（低偏差=高效率=高预期收益）"
else:
    print(f"   → 使用正向 (做多低效率/高偏差)")
    direction = 1
    fa_final = fa
    direction_desc = "正向（高偏差=低效率=高预期收益，信息不对称溢价）"
...
for test_window in [10, 40, 60]:
    oc_dev_test = oc_dev.rolling(test_window, min_periods=max(5, test_window//2)).mean()
    fraw_test = np.log(oc_dev_test.clip(lower=1e-8))
