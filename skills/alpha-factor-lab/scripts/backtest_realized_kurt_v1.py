...
# -*- coding: utf-8 -*-
"""
因子: realized_kurt_v1 — 已实现峰度因子
==========================================

方向: 高阶矩 / 尾部风险定价

构造:
  1. 过去20个交易日日收益率的超额峰度(excess kurtosis)
  2. excess_kurtosis = E[(r-μ)^4]/σ^4 - 3
  3. 成交额(log_amount_20d) OLS中性化
  4. 5%缩尾

逻辑:
  - 高峰度(尖峰肥尾): 极端收益事件频繁,大涨大跌都多
    → 类似"彩票"特征的另一维度(Bali et al. 2011)
    → 散户偏好极端波动(刺激感) → 高峰度被高估 → 后续收益低(反向)
  - 低峰度(扁平/接近正态): 收益分布规整,极端事件少
    → 被机构偏好(风控友好) → 后续收益更稳

  也可能:
  - 高峰度 → 信息事件密集 → 定价不充分 → 有alpha(正向)
  
  双向探索。

理论:
  - Dittmar (2002) "Nonlinear Pricing Kernels" — 高阶矩在定价核中显著
  - Conrad, Dittmar & Ghysels (2013) "Ex Ante Skewness and Expected Stock Returns" JF
  - Amaya et al. (2015) "Does Realized Skewness and Kurtosis Predict Cross-Section of Equity Returns?" JFE

与现有因子的区别:
  - Vol CV / Amplitude: 衡量波动幅度/稳定性(二阶矩)
  - 偏度: 三阶矩(不对称性)
  - 本因子: 四阶矩(尾部厚度)——另一个独立维度
  - CVaR: 只看下尾，峰度看双侧尾部
"""
...
# ────────────────── 因子构造 ──────────────────
print(f"[2] 构造已实现峰度因子 (window={WINDOW})...")

kurt_mat = np.full((n_dates, n_stocks), np.nan)
...
for i in range(WINDOW, n_dates):
    ...
    for j in range(n_stocks):
        ...
        if len(valid) >= 12:  # 至少12个有效观测
            std = np.std(valid, ddof=1)
            if std > 1e-8:
                mean = np.mean(valid)
                # 超额峰度 = E[(r-μ)^4]/σ^4 - 3
                kurt_mat[i, j] = np.mean((valid - mean) ** 4) / (std ** 4) - 3

factor_raw = pd.DataFrame(kurt_mat, index=dates, columns=stocks)

print(f"   非空率: {factor_raw.notna().mean().mean():.2%}")
print(f"   均值: {factor_raw.stack().mean():.4f}, std: {factor_raw.stack().std():.4f}")
print(f"   分位: 5%={factor_raw.stack().quantile(0.05):.4f}, "
      f"50%={factor_raw.stack().quantile(0.50):.4f}, "
      f"95%={factor_raw.stack().quantile(0.95):.4f}")
...
print(f"   正向 (高峰度=高收益): IC={pos_ic:.4f}, t={pos_t:.2f}, Sharpe={pos_sh:.4f}, Mono={pos_mono:.2f}")
print(f"   反向 (低峰度=高收益): IC={neg_ic:.4f}, t={neg_t:.2f}, Sharpe={neg_sh:.4f}, Mono={neg_mono:.2f}")
...
if neg_score > pos_score:
    ...
    direction = -1
    fa_final = -fa
    direction_desc = "反向（低峰度=高预期收益，做空尾部肥厚的股票）"
else:
    print(f"   → 使用正向 (做多高峰度)")
    direction = 1
    fa_final = fa
    direction_desc = "正向（高峰度=高预期收益）"
...
for fwd in [5, 10, 20]:
    ...
    gr_t, _, _ = compute_group_returns(fa_final, ra, N_GROUPS, fwd, COST)
    ...
    score_t = abs(sh_t) + abs(t_t) * 0.3
    results_fwd[fwd] = m_t
    print(f"   fwd={fwd}d: IC={ic_t_val:.4f}, t={t_t:.2f}, Sharpe={sh_t:.4f}, Mono={mono_t:.2f}")
    if score_t > best_score:
        best_score = score_t
        best_fwd = fwd

print(f"   最优前瞻窗口: {best_fwd}d")
...
for test_window in [10, 30, 40, 60]:
    kurt_test = np.full((n_dates, n_stocks), np.nan)
    min_obs = max(8, test_window * 3 // 4)
    for i in range(test_window, n_dates):
        ...
        for j in range(n_stocks):
            ...
            if len(valid) >= min_obs:
                ...
                if std > 1e-8:
                    mean = np.mean(valid)
                    kurt_test[i, j] = np.mean((valid - mean) ** 4) / (std ** 4) - 3
    
    kurt_df = pd.DataFrame(kurt_test, index=dates, columns=stocks)
    for d in dates:
        row = kurt_df.loc[d].dropna()
        if len(row) < 10:
            continue
        lo = row.quantile(0.05)
        hi = row.quantile(0.95)
        kurt_df.loc[d] = kurt_df.loc[d].clip(lo, hi)
    
    for d in dates:
        f_t = kurt_df.loc[d].dropna()
        ...
        try:
            ...
            kurt_df.loc[d, common_t] = fc - X @ beta
        except:
            pass
    
    cd = sorted(kurt_df.dropna(how="all").index.intersection(ra.index))
    cs = sorted(kurt_df.columns.intersection(ra.columns))
    if len(cd) < 50:
        continue
    
    fa_t = (direction * kurt_df).loc[cd, cs]
    ...
    print(f"   Window={test_window}d: IC={m_t2.get('ic_mean',0):.4f}, "
          ...
          f"Mono={m_t2.get('monotonicity',0):.2f}")
...
ma_vals_dict = {}
for w in ma_list:
    ma_vals_dict[w] = close_piv.rolling(w, min_periods=max(w//2, 3)).mean()
ma_norm = {w: ma_vals_dict[w] / close_piv for w in ma_list}
...
report = {
    "factor_id": FACTOR_ID,
    "factor_name": "已实现峰度 v1",
    "factor_name_en": "Realized Kurtosis v1",
    "category": "高阶矩/尾部风险",
    "description": f"过去{WINDOW}日日收益率的超额峰度(四阶中心矩/σ⁴-3)，衡量收益分布尾部厚度。成交额OLS中性化。",
    "hypothesis": "高峰度(尖峰肥尾)股票极端收益事件频繁,类似彩票特征,散户偏好→高估→后续收益低。低峰度股票收益分布规整,机构偏好→后续收益好。",
    "formula": f"neutralize(kurtosis(daily_ret, {WINDOW}d) - 3, log_amount_20d)",
    "direction": direction,
