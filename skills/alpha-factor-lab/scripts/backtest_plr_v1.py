...
# -*- coding: utf-8 -*-
"""
因子: profit_loss_ratio_v1 — 盈亏比因子
==========================================
方向: 收益分布偏态 / 风险溢价

构造:
  盈亏比 = sum(|正收益|, 20d) / sum(|负收益|, 20d)
  取负值 + 成交额中性化(OLS) + 5%缩尾
  
逻辑:
  - 盈亏比越低 = 跌大涨小(左尾肥) = 风险暴露大 = 要求补偿
  - 盈亏比越高 = 涨大跌小(右尾肥) = "彩票型" = 散户追捧→高估
  
理论: Bali et al.(2011) MAX/MIN effect → 左尾风险溢价(A股小盘股散户忽略)
"""
...
FACTOR_ID = "profit_loss_ratio_v1"
...
# ────────────────── 因子构造 ──────────────────
print(f"[2] 构造盈亏比因子 (window={WINDOW})...")
t0 = time.time()

ret_vals = ret_piv.values.astype(np.float64)  # (T, N)
ret_vals_clipped = np.clip(ret_vals, -0.11, 0.11)  # 涨停/跌停截断

# cumulative sum for rolling window
cumsum_pos = np.cumsum(np.where(np.isnan(ret_vals_clipped) | (ret_vals_clipped <= 0), 0, ret_vals_clipped), axis=0)
cumsum_neg = np.cumsum(np.where(np.isnan(ret_vals_clipped) | (ret_vals_clipped >= 0), 0, -ret_vals_clipped), axis=0)

# rolling(WINDOW) sum: pos_sum[i] = cumsum_pos[i] - cumsum_pos[i-WINDOW]
pos_sum = np.full(ret_vals.shape, np.nan)
neg_sum = np.full(ret_vals.shape, np.nan)
for i in range(WINDOW, len(dates)):
    pos_sum[i] = cumsum_pos[i] - cumsum_pos[i - WINDOW]
    neg_sum[i] = cumsum_neg[i] - cumsum_neg[i - WINDOW]

# 盈亏比 = sum(|正收益|)/sum(|负收益|)
plr = np.full(ret_vals.shape, np.nan)
plr[WINDOW:] = np.where(
    neg_sum[WINDOW:] > 1e-8,
    pos_sum[WINDOW:] / neg_sum[WINDOW:],
    np.where(pos_sum[WINDOW:] > 1e-8, 10.0, np.nan)
)

factor_raw = pd.DataFrame(plr, index=dates, columns=stocks)
...
print(f"   正向 (高盈亏比→高收益): IC={pos_ic:.4f}, Sharpe={pos_sh:.4f}")
print(f"   反向 (低盈亏比→高收益): IC={neg_ic:.4f}, Sharpe={neg_sh:.4f}")
print(f"   ({time.time()-t0:.1f}s)")

if neg_sh > pos_sh and abs(neg_ic) > abs(pos_ic):
    direction = -1
    fa_final = -fa
    direction_desc = "反向（低盈亏比=高预期收益，左尾风险溢价）"
    print(f"   → 使用反向 ✓")
elif pos_sh > neg_sh:
    direction = 1
    fa_final = fa
    direction_desc = "正向（高盈亏比=高预期收益）"
    print(f"   → 使用正向 ✓")
else:
    direction = -1
    fa_final = -fa
    direction_desc = "反向（低盈亏比=高预期收益）"
    print(f"   → 使用反向 ✓ (Sharpe接近, 选IC更好)")
...
report = {
    "factor_id": FACTOR_ID,
    "factor_name": "盈亏比 v1",
    "factor_name_en": "Profit/Loss Ratio v1",
    "category": "收益偏态/风险溢价",
    "description": f"过去{WINDOW}日正收益绝对值之和/负收益绝对值之和。低盈亏比=左尾风险大=后续获超额补偿。",
    "hypothesis": "低盈亏比的股票(跌大张小=频繁小跌+偶发大跌)暴露左尾风险,A股小盘股被散户忽略→风险补偿不足→后续收益更高。",
    "formula": f"neutralize(-sum(|ret|_pos, {WINDOW}d) / sum(|ret|_neg, {WINDOW}d), log_amount_20d)",
    "direction": direction,
