...
# -*- coding: utf-8 -*-
"""
因子: pv_corr_v1 — 收益率-成交量变化率相关性因子
==========================================
方向: 量价背离

构造:
  1. 计算每日log(amount)变化率
  2. 20日窗口内日收益率与log成交额变化率的滚动相关系数
  3. 负号（量价背离=做多信号）：量价相背离时做多(均值回复)
  4. 成交额中性化(OLS) + 5%缩尾

简化vs pv_divergence_v1:
  - pv_divergence_v1 用3成分复杂复合信号(方向+幅度+强度)
  - pv_corr_v1 只用纯滚动相关性，减法更简单直接
  - 核心假设: 量价相关的断裂(负相关=背离)本身就有预测力
...
理论依据:
  - Gervais, Kaniel & Mingelgrin (2001): volume-return cross-section
  - Llorente et al. (2002): dynamic volume-return relation
  - Brennan, Chordia & Subrahmanyam (1998): order flow and returns
"""
...
log_amt_chg = log_amount.diff()

# 20日 Rolling correlation(return, log_amount_change)
factor_raw = ret_piv.rolling(WINDOW, min_periods=15).corr(log_amt_chg)
...
# 正向(正量价相关→高收益) vs 反向(负量价相关=背离→高收益)
...
rank_ic_neg = compute_ic_dynamic(-fa, ra, FORWARD_DAYS, "spearman")
gr_neg, tn_neg, hi_neg = compute_group_returns(-fa, ra, N_GROUPS, REBALANCE_FREQ, COST)
...
print(f"[6b] 方向确认...")
print(f"   正向Sharpe={pos_sh:.4f} (高相关→高收益)")
print(f"   反向Sharpe={neg_sh:.4f} (低相关/背离→高收益)")

if neg_sh > pos_sh:
    direction = -1
    direction_desc = "反向（量价背离=高预期收益）"
    ic_series = ic_neg
...
report = {
    "factor_id": FACTOR_ID,
    "factor_name": "量价相关性 v1",
    "factor_name_en": "Volume-Price Correlation v1",
    "category": "量价/微观结构",
    "description": f"20日收益率与log成交额变化率的滚动相关系数。负号表示量价背离做多。简化版pv_divergence: 纯相关性代替三成分复合。",
    "hypothesis": "量价背离(负相关)的股票后续均值回复——价涨量缩=上涨乏力, 价跌量增=恐慌抛售后反弹。",
    "formula": f"-neutralize(corr(daily_ret, d(log_amount), {WINDOW}d), log_amount_20d)",
    "direction": direction,
