...
# -*- coding: utf-8 -*-
"""
因子: vp_sync_v1 — 量价同步性因子
==========================================
方向: 量价/趋势质量

构造:
  1. 近20日中，日收益率>0 且 成交量>MA5成交量 的天数占比（量升价涨）
     减去 日收益率<0 且 成交量>MA5成交量 的天数占比（量升价跌）
  2. 成交额中性化(OLS)
  3. 5%缩尾

逻辑:
  - 量价同步性高（放量涨、缩量跌）= 趋势健康 = 多方力量占优
  - 量价背离高（放量跌、缩量涨）= 趋势不健康 = 空方暗流
  - 做多量价同步高的股票
  
理论:
  - Llorente et al. (2002): 正的量收益自相关→信息驱动交易
  - Campbell, Grossman & Wang (1993): 成交量蕴含供需信息
  - Gervais, Kaniel & Mingelgrin (2001): 异常高成交量预测后续收益
"""
...
# 成交量的5日均值作为基准
vol_ma5 = volume_piv.rolling(5, min_periods=3).mean()

# 量升价涨: ret > 0 且 volume > MA5(volume)
bullish_sync = ((ret_piv > 0) & (volume_piv > vol_ma5)).astype(float)
# 量升价跌: ret < 0 且 volume > MA5(volume)  
bearish_sync = ((ret_piv < 0) & (volume_piv > vol_ma5)).astype(float)

# 20日窗口内量升价涨占比 - 量升价跌占比
factor_raw = bullish_sync.rolling(WINDOW, min_periods=10).mean() - bearish_sync.rolling(WINDOW, min_periods=10).mean()
...
if neg_sh > pos_sh:
    ...
    direction = -1
    direction_desc = "反向（低量价同步=高预期收益）"
else:
    direction = 1
    direction_desc = "正向（高量价同步=高预期收益，趋势健康溢价）"
    print(f"   → 使用正向 ✓")
...
tae_raw = np.log((turnover_piv.rolling(20, min_periods=10).mean() / (amplitude_piv.rolling(20, min_periods=10).mean() + 0.01)).clip(lower=1e-8))
...
report = {
    "factor_id": FACTOR_ID,
    "factor_name": "量价同步性 v1",
    "factor_name_en": "Volume-Price Synchronicity v1",
    "category": "量价/趋势质量",
    "description": "近20日中量升价涨频率减去量升价跌频率，衡量成交量与价格变动的同步程度。成交额中性化。",
    "hypothesis": "量价同步高=趋势健康=多方力量占优=后续收益好",
    "formula": f"neutralize(MA{WINDOW}(bullish_vol_up) - MA{WINDOW}(bearish_vol_up), log_amount_20d)",
    "direction": direction,
