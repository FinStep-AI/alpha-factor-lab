...
构造:
  1. 每日趋势效率 = |close - open| / (high - low + eps)
     - 值域 [0, 1]：1 = 完美趋势日(单边)，0 = 完美震荡日(十字星)
  2. 20日移动平均趋势效率
  3. 成交额中性化(OLS)
  4. 5%缩尾

逻辑:
  趋势效率高 = 日内价格从开盘到收盘单边运行，不来回震荡。
  这类股票的特征：
  → 信息释放有序，不是噪声交易主导
  → 主力资金有明确方向感
  → 价格发现效率高，动量效应强
  ...
  趋势效率低 = 日内大幅震荡但收盘回到开盘附近（十字星/长影线）。
  → 多空分歧大，信息含混
  → 噪声交易者占比高
  → 后续走势不确定性大

理论:
  - Amihud & Mendelson (1987): 日内价格效率反映市场微观结构质量
  - Chordia, Roll & Subrahmanyam (2011): 交易效率与定价效率正相关
  - 直觉类似 "趋势跟踪"：有方向感的股票后续动量更强
"""
...
# 每日趋势效率 = |close - open| / (high - low + eps)
eps = 1e-8
...
# 20日移动平均趋势效率
factor_raw = daily_trend_eff.rolling(WINDOW, min_periods=10).mean()
...
if neg_sh > pos_sh:
    ...
    direction = -1
    direction_desc = "反向（低趋势效率=高预期收益，震荡后反弹/均值回复）"
else:
    direction = 1
    direction_desc = "正向（高趋势效率=高预期收益，动量延续）"

print(f"   → 使用{'正向' if direction == 1 else '反向'} ✓")

# ────────────────── 也测试 20d 调仓 ──────────────────
print(f"[6c] 测试20d调仓频率...")
ic_20 = compute_ic_dynamic(fa, ra, 20, "pearson")
gr_20, turnovers_20, hi_20 = compute_group_returns(fa, ra, N_GROUPS, 20, COST)
m_20 = compute_metrics(gr_20, ic_20, ic_20, turnovers_20, N_GROUPS, holdings_info=hi_20)
sh_5 = metrics.get("long_short_sharpe", 0) or 0
sh_20 = m_20.get("long_short_sharpe", 0) or 0
print(f"   5d调仓 Sharpe={sh_5:.4f}, 20d调仓 Sharpe={sh_20:.4f}")
...
if sh_20 > sh_5 * 1.15:
    ...
    REBALANCE_FREQ = 20
    ic_series = ic_20
    rank_ic_series = compute_ic_dynamic(fa, ra, 20, "spearman")
    group_returns = gr_20
    turnovers = turnovers_20
    holdings_info = hi_20
    metrics = m_20
else:
    print(f"   → 保持5d调仓")
...
tae = np.log(turnover_piv.rolling(20, min_periods=10).mean() / (amplitude_piv.rolling(20, min_periods=10).mean() + 0.01).clip(lower=1e-8))
...
for name, other in [('amihud_illiq_v2', amihud_factor), ('shadow_pressure_v1', shadow),
                     ('overnight_momentum_v1', overnight_mom), ('tae_v1', tae),
                     ('turnover_level_v1', turnover_level), ('tail_risk_cvar_v1', cvar_df)]:
    corrs = []
...
report = {
    "factor_id": FACTOR_ID,
    "factor_name": "趋势日比率 v1",
    "factor_name_en": "Trend Day Ratio v1",
    "category": "量价/微观结构",
    "description": f"|close-open|/(high-low)的{WINDOW}日均值，成交额中性化。衡量日内价格运行效率——趋势日(单边走势)占比。",
    "hypothesis": "趋势效率高的股票信息释放有序，主力方向明确，后续动量延续性好。",
    "formula": f"neutralize(MA{WINDOW}(|close-open|/(high-low)), log_amount_20d)",
    "direction": direction,
...
print(f"\n{'='*60}")
print(f"  {FACTOR_ID}: 趋势日比率因子")
print(f"  方向: {direction_desc}")
print(f"  调仓: {REBALANCE_FREQ}d, 前瞻: {FORWARD_DAYS}d")
print(f"{'='*60}")
