...
# -*- coding: utf-8 -*-
"""
因子: price_mom_5d_v1 — 5日价格动量因子
==========================================
方向: 动量/反转

构造:
  1. 计算5日累计收益率 (纯价格动量，非隔夜/非日内分解)
  2. 成交额中性化(OLS) + 5%缩尾
  3. 正向和反向都测试，取更优方向

逻辑:
  纯5日动量信号，衡量股票近期涨跌方向。
  与existing factors的角度差异:
  - overnight_momentum_v1: 隔夜vs日内收益差 → 集合竞价信息效应
  - gap_momentum_v1: 跳空缺口方向一致性 → 隔夜信息方向
  - shadow_pressure_v1: 影线压力 → 单日微观结构
  - ma_disp_v1: 均线离散度 → 趋势确认
...
理论依据:
  - Jegadeesh (1990) / Lehmann (1990): 短期动量/反转
  - Rouwenhorst (1998): 交叉-sectional momentum
  - A股5日反转显著(屈文洲等 2007)
"""
...
# ────────────────── 因子构造 ──────────────────
print(f"[2] 构造5日价格动量因子...")

# 5日累计收益率
factor_raw = ret_piv.rolling(WINDOW, min_periods=3).sum()
...
print(f"[6b] 方向确认...")
print(f"   正向Sharpe={pos_sh:.4f} (高动量→高收益)")
print(f"   反向Sharpe={neg_sh:.4f} (低动量→高收益，反转)")

if neg_sh > pos_sh:
    direction = -1
    direction_desc = "反向（低动量=高预期收益，短期反转效应）"
    ic_series = ic_neg
    ...
    direction = 1
    direction_desc = "正向（高动量=高预期收益，动量延续效应）"
    ic_series = ic_pos
...
report = {
    "factor_id": FACTOR_ID,
    "factor_name": "5日价格动量 v1",
    "factor_name_en": "5-Day Price Momentum v1",
    "category": "动量/反转",
    "description": f"5日累计收益率, 成交额中性化。衡量近期价格方向。正向/反向分别测试。",
    "hypothesis": "近期上涨的股票倾向于继续上涨(动量延续)或均值回复(反转)。在A股小盘股上哪种效应占优需实证验证。",
    "formula": f"neutralize(sum(daily_ret, {WINDOW}d), log_amount_20d)",
    "direction": direction,
