...
FACTOR_ID = 'roe_reaccel_capdisc_v1'
...
fund['roe_yoy_lag2'] = g['roe_yoy'].shift(2)
fund['roe_reaccel'] = fund['roe_yoy'] - fund['roe_yoy_lag1']
fund['roe_reaccel2'] = fund['roe_reaccel'] - (fund['roe_yoy_lag1'] - fund['roe_yoy_lag2'])
...
fund['bps_expand_ma2'] = (fund['bps_yoy'].clip(lower=0).fillna(0) + 0.6 * fund['bps_qoq'].clip(lower=0).fillna(0)) / 1.6
fund['roe_roll_slope'] = g['roe'].transform(lambda s: s.diff().rolling(3, min_periods=2).mean())

# 核心思想：奖励“盈利同比改善再次加速”，但必须发生在 ROE 已为正/稳定、且净资产扩张不激进的公司里。
core = 0.65 * fund['roe_reaccel'].fillna(0) + 0.35 * fund['roe_reaccel2'].fillna(0)
profit_anchor = np.tanh(fund['roe_mean4'].fillna(0) / 7.5)
trend_anchor = np.tanh(fund['roe_roll_slope'].fillna(0) / 1.5)
stability_gate = 1.0 / (1.0 + 0.45 * fund['roe_std4'].abs().fillna(0) + 0.65 * fund['roe_yoy_std4'].abs().fillna(0))
capital_discipline = 1.0 / (1.0 + 4.5 * fund['bps_expand_ma2'].abs())

fund['raw_factor'] = core * (0.7 + 0.3 * profit_anchor) * (0.8 + 0.2 * trend_anchor) * stability_gate * capital_discipline
...
factor = pd.concat(panels, ignore_index=True)
...
def neutralize(vals, ctrl):
    ...
    out[np.where(mask)[0]] = (clipped - np.median(clipped)) / std
    return out

out = []
for date, grp in factor.groupby('date'):
    nz = neutralize(grp['raw_factor'].to_numpy(dtype=float), grp['log_mktcap'].to_numpy(dtype=float))
    good = np.isfinite(nz)
    if good.sum() == 0:
        continue
