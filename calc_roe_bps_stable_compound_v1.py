...
FACTOR_ID = 'roe_bps_stable_compound_v1'
...
fund['bps_yoy_std3'] = g['bps_yoy'].transform(lambda s: s.rolling(3, min_periods=2).std())
fund['bps_yoy_mean2'] = g['bps_yoy'].transform(lambda s: s.rolling(2, min_periods=2).mean())

# 核心：偏好 ROE 在改善、且 BPS 持续扩张但不过度跳跃的公司；
# 用稳定性门控，避免“靠激进扩表堆出来”的假成长。
roe_improve = 0.7 * fund['roe_yoy'].fillna(0) + 0.3 * fund['roe_qoq'].fillna(0)
roe_level = np.tanh(fund['roe_mean4'].fillna(0) / 8.0)
roe_stability = 1.0 / (1.0 + 0.55 * fund['roe_std4'].abs().fillna(0))

bps_growth = 0.75 * fund['bps_yoy'].clip(lower=-0.3, upper=1.5).fillna(0) + 0.25 * fund['bps_qoq'].clip(lower=-0.15, upper=0.5).fillna(0)
bps_stability = 1.0 / (1.0 + 3.5 * fund['bps_yoy_std3'].abs().fillna(0))
capital_discipline = 1.0 / (1.0 + 2.5 * fund['bps_yoy_mean2'].clip(lower=0.35).sub(0.35).clip(lower=0).fillna(0))

fund['raw_factor'] = (
    np.tanh(roe_improve / 4.5) *
    (0.55 + 0.45 * roe_level) *
    roe_stability *
    np.tanh(1.8 * bps_growth) *
    (0.6 + 0.4 * bps_stability) *
    capital_discipline
)
