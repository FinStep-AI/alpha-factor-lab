...
FACTOR_ID = 'roe_improve_smooth_guard_v1'
...
fund['roe_qoq_accel'] = fund['roe_qoq'] - fund['roe_prev_qoq']
...
fund['bps_growth_burst'] = fund[['bps_yoy','bps_qoq']].max(axis=1)

improve = np.tanh(fund['roe_yoy_mean2'].fillna(0) / 5.0)
level = np.tanh(fund['roe_mean4'].fillna(0) / 8.0)
reaccel = np.tanh(fund['roe_qoq_accel'].fillna(0) / 3.0)
smooth = 1.0 / (1.0 + 0.6 * fund['roe_std4'].abs().fillna(0) + 0.4 * fund['roe_yoy_std4'].abs().fillna(0))
capital_guard = 1.0 - np.tanh(2.5 * fund['bps_growth_burst'].clip(lower=0).fillna(0))

fund['raw_factor'] = (0.42 * improve + 0.28 * level + 0.18 * reaccel) * smooth * capital_guard
