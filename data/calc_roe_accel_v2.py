"""
ROE改善加速度因子 v2 (roe_accel_v2)
====================================
改进：
1. 同比delta-ROE
2. 加速度 = delta-ROE(t) - delta-ROE(t-1)
3. 截面rank变换（非线性变换，减轻极端值的影响，改善单调性）
4. 市值中性化
"""
import pandas as pd
import numpy as np
...
for rd in report_dates:
    ...
    # 截面rank标准化到[-1, 1]
    ranked = vals.rank(pct=True) * 2 - 1  # [-1, 1]
    
    print(f"  {rd} → {avail_date}, N={len(ranked)}")
    
    for sc, v in ranked.items():
...
output.to_csv('data/factor_roe_accel_v2.csv', index=False)
print("✅ 已保存")
