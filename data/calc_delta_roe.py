"""
ROE同比变化因子 (delta_roe_v1)
================================
逻辑：
1. 取每个报告期的ROE
2. 计算同比delta-ROE = ROE(t) - ROE(t-4)
3. 映射到日频，市值中性化
信号：delta-ROE > 0 → 盈利同比改善 → 正向因子
Barra: Growth
"""
import pandas as pd
import numpy as np
...
for rd in report_dates:
    ...
    if mm_dd in report_to_available:
        ...
        continue
    if rd not in delta_roe.columns:
        continue
    vals = delta_roe[rd].dropna()
    if len(vals) == 0:
        continue
    print(f"  报告期 {rd} → {avail_date}, N={len(vals)}")
...
output.to_csv('data/factor_delta_roe.csv', index=False)
print("✅ 已保存 data/factor_delta_roe.csv")
