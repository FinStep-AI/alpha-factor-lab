...
FACTOR_ID = 'asset_turnover_reaccel_quality_v2'
...
fund['at'] = fund['roe'] / fund['bps'].clip(lower=0.1)
...
fund['bps_growth'] = g['bps'].pct_change()

fund['at_yoy'] = fund['at'] - fund['at_l4']
fund['at_reaccel'] = (fund['at'] - fund['at_l4']) - (fund['at_l4'] - fund['at_l8'])
fund['at_qoq'] = fund['at'] - fund['at_l1']
fund['roe_yoy'] = fund['roe'] - fund['roe_l4']
fund['roe_qoq'] = fund['roe'] - fund['roe_l1']

improve_core = np.tanh(22.0 * fund['at_yoy'].fillna(0))
reaccel_core = np.tanh(28.0 * fund['at_reaccel'].fillna(0))
qoq_confirm = np.tanh(16.0 * fund['at_qoq'].fillna(0))
profit_confirm = np.tanh((0.8 * fund['roe_yoy'].fillna(0) + 0.2 * fund['roe_qoq'].fillna(0)) / 4.5)
stability = 1.0 / (1.0 + 0.22 * fund['roe_std4'].abs().fillna(0) + 4.0 * fund['at_std4'].abs().fillna(0))
capital_discipline = 1.0 / (1.0 + 10.0 * fund['bps_growth'].clip(lower=0).fillna(0).abs())
low_roe_penalty = 0.55 + 0.45 * np.tanh(fund['roe'].fillna(0) / 7.0)
...
fund['raw_factor'] = (
    ...
    0.16 * qoq_confirm +
    0.20 * profit_confirm
) * stability * capital_discipline * low_roe_penalty
