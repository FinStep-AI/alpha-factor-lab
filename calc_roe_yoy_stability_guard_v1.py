...
FACTOR_ID = 'roe_yoy_stability_guard_v1'
...
signal = np.tanh(fund['roe_yoy_mean2'].fillna(0) / 4.0)
profit_gate = np.tanh(fund['roe_mean4'].fillna(0) / 10.0)
stability = 1.0 / (1.0 + fund['roe_std4'].abs().fillna(0))
expansion_penalty = 1.0 / (1.0 + 3.0 * fund['bps_yoy'].clip(lower=0).fillna(0).abs())
fund['raw_factor'] = signal * (0.65 + 0.35 * profit_gate) * stability * expansion_penalty
