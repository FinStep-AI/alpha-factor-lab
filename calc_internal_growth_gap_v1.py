...
FACTOR_ID = 'internal_growth_gap_v1'
...
fund['bps_growth_std4'] = g['bps_yoy'].transform(lambda s: s.rolling(4, min_periods=3).std())
fund['roe_persist'] = -(fund['roe_qoq'].abs())

for col in ['roe_mean4','roe_std4','roe_yoy','bps_yoy','bps_growth_std4','roe_persist']:
    s = fund[col]
    q01, q99 = s.quantile([0.01, 0.99])
    fund[col] = s.clip(q01, q99)

internal_growth = np.tanh(fund['roe_mean4'].fillna(fund['roe']).fillna(0.0) / 10.0)
balance_sheet_growth = np.tanh(fund['bps_yoy'].fillna(0.0) / 0.25)
roe_persistence = np.tanh(fund['roe_persist'].fillna(0.0) / 3.0)
roe_improve = np.tanh(fund['roe_yoy'].fillna(0.0) / 5.0)
growth_stability_penalty = np.tanh(fund['bps_growth_std4'].fillna(0.0) / 0.20)
roe_vol_penalty = np.tanh(fund['roe_std4'].fillna(0.0) / 4.0)

fund['raw_factor'] = (
    0.45 * (internal_growth - balance_sheet_growth) +
    0.20 * roe_persistence +
    0.20 * roe_improve -
    0.10 * growth_stability_penalty -
    0.05 * roe_vol_penalty
)
...
print(f'saved {OUT} rows={len(result)} unique_dates={result.date.nunique()} stocks={result.stock_code.nunique()}')
