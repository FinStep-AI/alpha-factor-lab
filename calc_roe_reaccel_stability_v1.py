...
FACTOR_ID = 'roe_reaccel_stability_v1'
...
# Growth + Quality: accelerating ROE improvement, but only when ROE level is positive and stable.
level_gate = np.sign(fund['roe_mean4']) * np.log1p(fund['roe_mean4'].abs().clip(lower=0))
stability = 1.0 / (1.0 + fund['roe_std4'].abs() + 0.5 * fund['roe_yoy_std4'].abs())
reaccel = fund['roe_yoy_accel']
turnaround_bonus = np.tanh(fund['roe_yoy'].fillna(0) / 5.0)
fund['raw_factor'] = reaccel * level_gate * stability * (1.0 + 0.5 * turnaround_bonus)
