...
FACTOR_ID = 'gross_margin_proxy_growth_v1'
...
# clip fundamentals for noisy small caps
for col in ['roe','bps']:
...
# proxy intuition: in absence of explicit gross margin, require equity/book growth to be positive but smooth,
# while ROE improvement must come from already-profitable, stable businesses.
quality_gate = np.sign(fund['roe_mean4']) * np.log1p(fund['roe_mean4'].abs().clip(lower=0))
roe_stability = 1.0 / (1.0 + fund['roe_std4'].abs())
growth_quality = fund['bps_yoy_mean2'] / (1.0 + 2.0*fund['bps_yoy_std4'].abs())
fund['raw_factor'] = fund['roe_yoy'] * quality_gate * roe_stability * np.tanh(3.0 * growth_quality.fillna(0))
