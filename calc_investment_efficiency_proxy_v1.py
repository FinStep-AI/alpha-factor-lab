...
FACTOR_ID = 'investment_efficiency_proxy_v1'
...
# trim extreme accounting outliers
for col in ['roe', 'bps']:
...
# academic intuition from investment style/profitability literature:
# high profitability improvement with restrained balance-sheet expansion is better.
fund['raw_factor'] = fund['roe_yoy'] / (fund['bps_yoy'].abs() + 0.08)
fund['raw_factor'] = fund['raw_factor'] * np.sign(fund['roe_ma4']) * np.log1p(fund['roe_ma4'].abs().clip(lower=0))
fund['raw_factor'] = fund['raw_factor'] / (1.0 + fund['roe_std4'].fillna(fund['roe_std4'].median()).abs())
