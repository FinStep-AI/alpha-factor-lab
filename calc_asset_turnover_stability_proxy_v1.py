...
FACTOR_ID = 'asset_turnover_stability_proxy_v1'
...
fund['roe_std8'] = g['roe'].transform(lambda s: s.rolling(8, min_periods=5).std())
fund['bps_yoy_mean2'] = g['bps'].transform(lambda s: s.pct_change(4).rolling(2, min_periods=2).mean())

fund['roe_yoy'] = fund['roe'] - fund['roe_lag4']
fund['bps_yoy'] = fund['bps'] / fund['bps_lag4'] - 1.0
fund['bps_qoq'] = fund['bps'] / fund['bps_lag1'] - 1.0

turnover_proxy = fund['roe_yoy'] - 55.0 * fund['bps_yoy'].fillna(0.0) - 20.0 * fund['bps_qoq'].fillna(0.0)
profitability_anchor = np.tanh(fund['roe_mean4'].fillna(0.0) / 7.5)
stability_gate = 1.0 / (1.0 + 0.7 * fund['roe_std4'].abs().fillna(fund['roe_std8'].abs()) + 0.3 * fund['roe_std8'].abs())
growth_discipline = 1.0 / (1.0 + 8.0 * fund['bps_yoy_mean2'].clip(lower=0).fillna(0.0).abs())

fund['raw_factor'] = turnover_proxy * profitability_anchor * stability_gate * growth_discipline
